...
class FJSPEnv(gym.Env):
    ...
    def __init__(self, case, env_paras, data_source='case'):
        ...
        curr_proc_batch = self.ope_ma_adj_batch[batch_indices, start_opes]

        mean_proc_time = torch.sum(self.proc_times_batch, dim=-1).div(torch.count_nonzero(self.ope_ma_adj_batch, dim=-1) + 1e-9)   
        cum_time = torch.cumsum(mean_proc_time, dim=-1)

        feat_opes_batch[..., 0] = torch.count_nonzero(curr_proc_batch, dim=-1)
        ...
        feat_opes_batch[..., 6] = self.nums_ope_batch
        feat_opes_batch[..., 7] = feat_opes_batch[..., 5]                
                   

        feat_macs_batch[..., 0] = torch.count_nonzero(self.ope_ma_adj_batch, dim=-2)
        feat_macs_batch[..., 1] = torch.sum(self.proc_times_batch, dim=-2).div(feat_macs_batch[..., 0] + 1e-9)
    ...
    def step(self, actions):
        ...
        self.curr_proc_batch[self.batch_idxes, jobs, :] = self.ope_ma_adj_batch[self.batch_idxes, next_opes, :]

        self.feat_opes_batch[self.batch_idxes, jobs, 0] = torch.count_nonzero(self.curr_proc_batch[self.batch_idxes, jobs, :], dim=-1).float()
        ...
        self.feat_opes_batch[self.batch_idxes, jobs, 6] -= 1                            
        self.feat_opes_batch[self.batch_idxes, jobs, 7] = total_sum
        ...
        self.feat_macs_batch[self.batch_idxes, :, 1] = torch.sum(self.proc_times_batch[self.batch_idxes, ...], dim=-2) \
                                                       .div(self.feat_macs_batch[self.batch_idxes, :, 0] + 1e-9)
        self.feat_macs_batch[self.batch_idxes, mas, 2] = self.time[self.batch_idxes] + self.proc_times_batch[self.batch_idxes, opes, mas]
        self.feat_macs_batch[self.batch_idxes, :, 3] = self.mask_ma_procing_batch[self.batch_idxes, :].float()
